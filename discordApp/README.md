...
Discord内などでカーソル固定が許可されない場合は、クリックした位置を基準にカーソルの位置で操縦します。カーソルを画面内の端に寄せたままにすると旋回を続けられ、Escで解除できます。カーソルがActivityの外に出た間は操縦を止め、戻すと再開します。

同じ設定画面でゲーム音量を0～100%に調整できます。エンジン、機銃、敵への命中、自機の被弾、ミサイル発射、ロックオン、フレア、爆発に効果音があります。効果音は音量を高めに調整し、重なった音は急に大きくならないよう制御します。自機を追尾するロックオンミサイルが飛来すると、機体に対する方向と距離をHUDに表示し、警告音を鳴らします。フレアで追尾が解除されると警告も消えます。
